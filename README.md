...
| 番号 | 0 | 1 | 2 | 3 | 4 | 5 |
|---|---|---|---|---|---|---|
| 面 | U 上 | F 前 | R 右 | B 後 | L 左 | D 下 |

上が `0`、下が `5`。あいだの `1`〜`4` は、F から始めて R → B → L と
キューブを右回りに一周する順。

それぞれの面の向きは、紙に描いて折りたたむと立方体になる
標準的な展開図の向きで決めてある。
...
まわりの4面 `F, R, B, L` の座標がぐるっと一周そろうので、`U` と `D` の操作が
...
# してください。cube[1][1][0] (F面の 縦1 横0) が 'X' になっています。
```
...
W B O    G W Y    R R W    B Y R    B O O    Y G R
W B G    G W W    B R R    B Y B    O O O    Y G Y
W R G    G W G    W R R    B Y B    O O O    Y G Y
```
...
| ファイル | 中身 |
|---|---|
| `src/rubik/state.py` | リスト表現、`solved()`、`show()`、`net()`、`faces()`、形の検査 |
